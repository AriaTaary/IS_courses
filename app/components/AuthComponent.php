...
class AuthComponent extends Component
{
     ...
     * @return bool
     */
    public function auth($email, $password, $remember = 0)
    {
    ...
    public function update($id, $email, $name, $surname, $fathername, $status_id)
    {
        $this->usersTable->beginTransaction();
        $statusComponent = new StatusComponent();
        ...
        if (!empty($statusComponent->getById($status_id))) {
            ...
            if ($edit = $this->usersTable->update(['email' => $email, 'name' => $name, 'surname' => $surname, 'fathername' => $fathername], ['id' => $id])) {
                $this->usersTable->commit();
                return true;
            } else {
                $this->usersTable->rollBack();
    ...
    public function setSession($auth_token)
    {
        ...
        App::$session->user->setRole($user['status']);
